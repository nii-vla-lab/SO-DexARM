...
class SO101Kinematics:
    ...
    def forward_tcp_pose(self, joint_degrees: Mapping[str, float], tcp_offset: TcpOffset | None = None) -> np.ndarray:
    ...
    def solve_tcp_ik(
        ...
        *,
        ...
        pan_rad = math.atan2(y, x) if radial > 1e-9 else math.radians(
            float((current_joint_degrees or {}).get("shoulder_pan", 0.0))
        )
...
class SO101LeRobotCalibration:
    ...
    def from_file(
        ...
        *,
        ...
        return cls(data if isinstance(data, dict) else {}, action_units=action_units, max_position=max_position)
    ...
    def clamp_action(self, action: Mapping[str, float]) -> tuple[dict[str, float], dict[str, tuple[float, float, float]]]:
