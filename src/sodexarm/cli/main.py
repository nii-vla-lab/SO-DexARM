#!/usr/bin/env python
"""Bimanual SO-101 + AmazingHand pipeline CLI: calibrate / teleop / record / train / eval.

Originally scripts/quest_hts/phase32_bimanual_calibrate_teleop.py, integrated into the
lerobot package as the single entry point for the Quest-teleoperated bimanual rig.
...
teleop          -- Start bimanual teleoperation (Meta Quest → robot)
...
# 3. Capture start positions (move arms to desired start pose, then run)
...
# Public defaults use stable, physical-side udev names. Installation-specific
# swaps and nonstandard motor IDs remain CLI overrides rather than source defaults.
...
# Camera serial numbers are installation-specific and must be supplied explicitly.
DEFAULT_CAMERAS = None

# Unified calibration dir for BiSO101AmazingHandFollower (r.json + l.json)
DEFAULT_BI_CALIB_DIR = Path(".cache/calibration/robots/bi_so101_amazinghand_follower")

DEFAULT_RUNTIME_CONFIG_DIR = Path(".cache/so_dexarm")
DEFAULT_HAND_CALIB_FILE = DEFAULT_RUNTIME_CONFIG_DIR / "quest_hts_bimanual_hand_calibration.yaml"
DEFAULT_STARTUP_FILE = DEFAULT_RUNTIME_CONFIG_DIR / "quest_hts_bimanual_startup.yaml"
...
def _parse_motor_ids(value: str) -> tuple[int, ...]:
    ids = tuple(int(item.strip()) for item in value.strip("[]()").split(",") if item.strip())
    if len(ids) != 5 or len(set(ids)) != 5:
        raise argparse.ArgumentTypeError("motor IDs must be five distinct comma-separated integers")
    return ids
...
class _ArmAdapter:
    ...
    def connect(self) -> None:
        ...
        from sodexarm.robots.so101_amazinghand_right import SO101AmazingHandRightConfig
...
class _HandAdapter:
    ...
    # SCS0009 raw tick range (from BiSO101AmazingHandFollowerConfig defaults)
    _RANGE_MIN = 200
    _RANGE_MAX = 800
...
class _QuestHTSSession:
    ...
    def wait_for_any_data(self, *, timeout_s: float = 30.0) -> dict[str, list[str]]:
        ...
        while time.monotonic() < deadline:
            with self._lock:
                received = {
                    side: list(data.keys())
                    for side, data in self._latest.items()
                    if data
                }
            if received:
                return received
    ...
    def _loop(self) -> None:
        from sodexarm.teleoperators.quest_hts.hts_protocol import parse_hts_line
                    ...
                    while "\n" in buf:
                        ...
                        with self._lock:
                            ...
                            if not first_frame_logged and kind == "landmarks":
                                ...
                                print(f"  [HTS] first {side} landmarks received ({len(values)} values)", flush=True)
            finally:
                try:
                    conn.close()
                except OSError:
...
def cmd_setup_motors(args: argparse.Namespace) -> int:
    """Assign the five SO-101 arm IDs, one physically connected motor at a time."""
    from lerobot.motors import Motor, MotorNormMode

    from sodexarm.motors.feetech import FeetechMotorsBus
    ...
    print("\nConnect exactly ONE motor for each prompt.\n", flush=True)
...
def cmd_calibrate_arm(args: argparse.Namespace) -> int:
    ...
    # On the shared single-port bus the right arm is renumbered to 21-25; left keeps 11-15. The
    # calibrate robot must address THIS side's IDs or it would sweep the wrong arm on the shared bus.
    ...
    cmd = [
        sys.executable, "-m", "sodexarm.control.calibrate",
        "--robot.type", "so101_amazinghand_right",
        "--robot.id", robot_id,
        "--robot.port", port,
        f"--robot.arm_motor_ids={ids_arg}",
        "--robot.mode", "arm-only",
        "--robot.calibration_dir", str(calib_dir),
    ]
    rc = _run(cmd)
    if rc == 0:
        _sync_bi_calib_files(args)
    return rc
...
def cmd_calibrate_hand(args: argparse.Namespace) -> int:
    ...
    print(f"\n=== Calibrate hand: robot_side={side}  quest_side={quest_side}  poses={poses_to_capture} ===", flush=True)
    ...
    from sodexarm.teleoperators.quest_hts.bimanual_calibration import (
    ...
    with _QuestHTSSession(host=args.hts_host, port=args.hts_port) as session:
        ...
        for pose in poses_to_capture:
            ...
            # Save to YAML
            save_human_hand_pose(
                path=hand_calib_file, side=side, pose=pose, landmarks=landmarks,
...
def cmd_capture_startup(args: argparse.Namespace) -> int:
    ...
    for side in sides:
        ...
        from sodexarm.teleoperators.quest_hts.bimanual_calibration import save_start_position
        save_start_position(
            path=startup_file,
            side=side,
    ...
    # Sync arm calibration files to bi_so101_amazinghand_follower format
...
def _sync_bi_calib_files(args: argparse.Namespace) -> None:
    """Refresh the bi-level calibration after an arm recalibration: per-arm r.json/l.json AND the
    merged bi-level file, keeping the merged file's ARM half FRESH and its HAND half intact.

    Why this is delicate (BiSO101AmazingHandFollower behaviour):
      - It loads the merged calibration_fpath ('{id}.json'; id defaults to None → None.json) and
        ...
        calibration → the arm flies off at move-to-start (seen after the 12V swap, 2026-06-23).
    ...
    import json
    import shutil
    bi_dir = getattr(args, "bi_calib_dir", DEFAULT_BI_CALIB_DIR)
    bi_dir.mkdir(parents=True, exist_ok=True)
    mapping = {
        args.right_calib_dir / "right.json": bi_dir / "r.json",
        args.left_calib_dir  / "left.json":  bi_dir / "l.json",
    }
    ...
    # Rebuild the merged bi-level file with FRESH arm + PRESERVED hand.
    merged: dict = {}
    for side_file, prefix in ((bi_dir / "r.json", "r_"), (bi_dir / "l.json", "l_")):
        if side_file.exists():
            with side_file.open(encoding="utf-8") as stream:
                side_calibration = json.load(stream)
    ...
    hand_kept = 0
    candidates = ["None.json", "bi_amazinghand_follower.json"]
    candidates += sorted((p.name for p in bi_dir.glob("*.stale-bak-*")), reverse=True)
    ...
    if hand_kept == 0:
        print("  WARNING: no HAND calibration found to preserve — run calibrate-hand "
              "(the merged file will lack finger calibration and teleop will fail on the hand).", flush=True)

    # Write to the loaded fpath (id=None → None.json) and the legacy name twin so either is current.
    for out in ("None.json", "bi_amazinghand_follower.json"):
        with (bi_dir / out).open("w", encoding="utf-8") as f:
            json.dump(merged, f, indent=4)
    print(f"  Rebuilt merged calibration ({len([k for k in merged if 'finger' not in k])} arm + "
          f"{hand_kept} hand) → {bi_dir/'None.json'}", flush=True)
...
def _teleop_args(args: argparse.Namespace) -> list[str]:
    ...
    BiSO101AmazingHandFollower internally creates SO101AmazingHandFollower with
    id="r" and id="l", so it looks for r.json / l.json in calibration_dir.
    We copy right.json → r.json and left.json → l.json into DEFAULT_BI_CALIB_DIR.
    """
    bi_calib_dir = getattr(args, "bi_calib_dir", DEFAULT_BI_CALIB_DIR)
    out = [
        "--robot.type", "bi_so101_amazinghand_follower",
        "--robot.port_r", args.right_arm_port,
        "--robot.port_l", args.left_arm_port,
        "--robot.hand_port_r", args.right_hand_port,
        "--robot.hand_port_l", args.left_hand_port,
        f"--robot.arm_ids_r=[{','.join(str(value) for value in args.right_arm_ids)}]",
        f"--robot.arm_ids_l=[{','.join(str(value) for value in args.left_arm_ids)}]",
        "--robot.calibration_dir", str(bi_calib_dir),
        "--teleop.type", "quest_hts_bimanual",
        "--teleop.host", args.hts_host,
        "--teleop.port", str(args.hts_port),
        "--teleop.startup_file", str(args.startup_file),
        "--teleop.hand_calib_file", str(args.hand_calib_file),
        "--teleop.right_arm_calib_file", str(args.right_calib_dir / "right.json"),
        "--teleop.left_arm_calib_file",  str(args.left_calib_dir / "left.json"),
        "--teleop.mode", getattr(args, "teleop_mode", "arm-and-hand"),
    ]
    # Single-side mode (e.g. only the LEFT arm+hand is physically connected). "both" = full
    # bimanual (default). Passing left/right makes BOTH the follower and the teleoperator expose +
    ...
    if getattr(args, "swap_arm_side", False):
        out += ["--teleop.swap_arm_side", "true"]
...
def _build_teleop_cmd(args: argparse.Namespace, *, extra: list[str] | None = None) -> list[str]:
    cmd = [sys.executable, "-m", "sodexarm.control.teleoperate"] + _teleop_args(args)
...
def cmd_record(args: argparse.Namespace) -> int:
    ...
    print(f"\n=== Bimanual Record: {args.repo_id} ===", flush=True)
    cmd = (
        [sys.executable, "-m", "sodexarm.control.record"]
        + _teleop_args(args)
        + [
            "--dataset.repo_id", args.repo_id,
            "--dataset.single_task", getattr(args, "single_task", "bimanual task") or "bimanual task",
            "--dataset.num_episodes", str(args.num_episodes),
            "--dataset.episode_time_s", str(args.episode_time_s),
            "--dataset.reset_time_s", str(args.reset_time_s),
            "--dataset.fps", str(getattr(args, "fps", 30)),
            "--dataset.push_to_hub", "true" if args.push_to_hub else "false",
        ]
    )
...
def cmd_eval(args: argparse.Namespace) -> int:
    ...
    cmd = [
        ...
        "--robot.calibration_dir", str(getattr(args, "bi_calib_dir", DEFAULT_BI_CALIB_DIR)),
        ...
        "--dataset.repo_id", getattr(args, "repo_id", None) or "masuzushi28/eval_result",
        "--dataset.single_task", getattr(args, "task", None) or "eval",
        "--dataset.num_episodes", str(getattr(args, "num_episodes", 5)),
        "--dataset.episode_time_s", str(getattr(args, "episode_time_s", 30)),
        "--dataset.reset_time_s", str(getattr(args, "reset_time_s", 10)),
        "--dataset.fps", str(getattr(args, "fps", 30)),
    ...
    # same configuration as record. --cameras none only works for a state-only policy.
    cams = (getattr(args, "cameras", "") or "").strip()
    if cams and cams.lower() != "none":
        cmd += ["--robot.cameras", cams]
        print(f"  Cameras: {cams}", flush=True)
    ...
    # inference stall make the arms jump at full servo speed (violent lunge, カクカク).
    if getattr(args, "max_relative_target", None) is not None:
        cmd += [f"--robot.max_relative_target={args.max_relative_target}"]
    if getattr(args, "right_hand_id8_inverted", False):
        cmd += ["--robot.right_hand_id8_inverted=true"]
    if getattr(args, "left_hand_id8_inverted", False):
        cmd += ["--robot.left_hand_id8_inverted=true"]
    if getattr(args, "right_hand_placefruit_compat", False):
        cmd += ["--robot.right_hand_placefruit_compat=true"]
    if getattr(args, "left_hand_placefruit_grip_gain", None) is not None:
        cmd += [
            f"--robot.left_hand_placefruit_grip_gain={args.left_hand_placefruit_grip_gain}"
        ]
    # Diffusion-policy latency: the stored default (num_inference_steps=None → num_train_timesteps
    # =100 denoise steps) measures ~1.57 s per action chunk on this machine → the robot freezes
    # between chunks. DDIM + 10 steps ≈ 160 ms. NOTE: non-diffusion policies (SmolVLA/ACT) REJECT
...
def cmd_map_hand(args: argparse.Namespace) -> int:
    from sodexarm.teleoperators.quest_hts.bimanual_calibration import build_hand_mapping
...
def _add_common_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--bi-calib-dir", type=Path, default=DEFAULT_BI_CALIB_DIR)
    ...
    parser.add_argument("--tcp-offset-m", type=float, default=None,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Bimanual SO-101 + AmazingHand: calibration, teleoperation, recording, training, evaluation.",
    ...
    # -- setup-motors --
    p = sub.add_parser("setup-motors", help="Assign the SO-101 motor IDs for one side.")
    _add_common_args(p)
    p.add_argument("--side", choices=("right", "left"), required=True)
    p.add_argument("--yes", action="store_true", help="Do not pause before each motor.")
    ...
    # -- calibrate-hand --
    p = sub.add_parser("calibrate-hand",
                       help="Capture hand poses (open=パー, mid=中間, fist=グー) for robot + human.")
    _add_common_args(p)
    p.add_argument("--side",  choices=("right", "left"), required=True,
                   help="Which robot hand to calibrate.")
    p.add_argument("--quest-side", choices=("right", "left"), default=None,
    ...
    p.add_argument("--pose",  choices=POSES, default="open",
                   help="Single pose to capture (ignored if --all-poses). "
                        "open=パー, mid=中間, fist=グー.")
    p.add_argument("--all-poses", action="store_true",
                   help="Capture all 3 poses (open → mid → fist = パー → 中間 → グー) in one "
                        "Quest session.")
    p.add_argument("--from-hardware", action="store_true",
                   help="Read current robot hand positions from hardware.")
    p.add_argument("--hand-feature-mode", choices=("curl", "distance"), default="curl",
    ...
    # -- teleop --
    p = sub.add_parser("teleop", help="Start bimanual teleoperation.")
    _add_common_args(p)
    p.add_argument("--sides", choices=("both", "right", "left"), default="both",
                   help="Which side(s) to drive. 'both' (default) = full bimanual. 'left'/'right' = "
                        "single-arm: only that side's arm+hand is connected and driven (use when only "
                        "one side is physically plugged in).")
    p.add_argument("--teleop-mode", default="arm-and-hand",
                   choices=("arm-only", "hand-only", "arm-and-hand"))
    ...
    p.add_argument("--swap-arm-side", action="store_true",
                   help="Swap ONLY the arm/wrist channel's left-right label, leaving the hand (finger) "
                        "channel untouched. Use when the Quest HTS Streamer app mislabels just the "
                        "wrist lines (arm crossed but hand correct) — swap_sides would wrongly flip "
                        "the hand too in that case.")
    ...
    p.add_argument("--single-task",   default="bimanual task",
    ...
    p.add_argument("--cameras", default=DEFAULT_CAMERAS,
                   help="draccus dict-string passed to --robot.cameras. No camera is selected by "
                        "default because serial numbers are installation-specific. Use 'none' (or "
                        "'') to record state-only with no images.")
    ...
    p.add_argument("--swap-arm-side", action="store_true",
                   help="Swap ONLY the arm/wrist channel's left-right label, leaving the hand (finger) "
                        ...
                        "the hand too in that case.")
    p.add_argument("--sides", choices=("both", "right", "left"), default="both",
                   help="Which side(s) to record. 'both' (default) = full bimanual. 'left'/'right' = "
    ...
    p.add_argument("--cameras", default=DEFAULT_CAMERAS,
                   help="REQUIRED for a visuomotor policy; pass the same camera configuration used "
                        "for recording. Pass 'none' only for a state-only policy.")
    p.add_argument("--display", action="store_true",
                   help="Show the Rerun viewer (camera feed + state) during eval.")
    ...
    p.add_argument("--right-hand-id8-inverted", action="store_true",
                   help="Eval compatibility shim for old policies after replacing the right AmazingHand "
                        "ID8 servo: maps r_finger4_motor2 normalized values as 100-value.")
    p.add_argument("--left-hand-id8-inverted", action="store_true",
                   help="Same compatibility shim for the left AmazingHand ID8 servo.")
    p.add_argument("--right-hand-placefruit-compat", action="store_true",
                   help="Restore the pre-2026-07-30 right thumb/index coordinate used by the "
                        "PlaceFruit training data, without changing calibration files.")
    p.add_argument("--left-hand-placefruit-grip-gain", type=float, default=None,
                   help="PlaceFruit-only left-hand closure multiplier. 1.25 is a conservative "
                        "starting point; omitted leaves the policy output unchanged.")
    ...
    p.add_argument("--noise-scheduler", choices=("DDPM", "DDIM"), default=None,
