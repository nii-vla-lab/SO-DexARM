#!/usr/bin/env python
"""Bimanual Quest HTS teleoperator.
...
from sodexarm.model.so101_kinematics import (
    SO101Kinematics,
...
# MEASURED empirically with axis_map_diagnostic.py --capture (2026-06-11):
...
# The previous matrix had x/z swapped (assumed x=forward, z=lateral) → "move hand right = arm
# reaches forward", "move hand forward = arm pans sideways". Correct mapping (robot = R @ quest):
...
class _SideState:
    ...
    hand_mapper: Any = None                             # PiecewiseHandMapper (bimanual_calibration)
    hand_start_action: dict[str, float] = field(default_factory=dict)  # prefixed hand keys at start
    hand_open_action: dict[str, float] = field(default_factory=dict)   # prefixed hand keys for "open"(パー) pose
    hand_fist_action: dict[str, float] = field(default_factory=dict)   # prefixed hand keys for "fist"(グー) pose — grip over-close reference
...
def _scale_rotation(R: np.ndarray, scale: float) -> np.ndarray:
    """Interpolate rotation R from identity by factor `scale` via angle-axis SLERP.
    ...
    if abs(scale - 1.0) < 1e-9:
        return R.copy()
    cos_a = max(-1.0, min(1.0, (float(np.trace(R)) - 1.0) / 2.0))
    angle = math.acos(cos_a)
    if angle < 1e-9:
        return np.eye(3, dtype=float)
    sin_a = math.sin(angle)
    ax = float(R[2, 1] - R[1, 2]) / (2.0 * sin_a)
    ay = float(R[0, 2] - R[2, 0]) / (2.0 * sin_a)
    az = float(R[1, 0] - R[0, 1]) / (2.0 * sin_a)
    sa = angle * scale
...
class QuestHTSBimanualTeleoperator(Teleoperator):
    """26-DOF bimanual teleoperator driven by Meta Quest hand tracking (TCP)."""

    config_class = QuestHTSBimanualTeleoperatorConfig
    name = "quest_hts_bimanual"

    def __init__(self, config: QuestHTSBimanualTeleoperatorConfig):
    ...
    def _init_side(self, state: _SideState) -> None:
        side = state.side
        calib_file = (
            self.config.right_arm_calib_file if side == "right"
            else self.config.left_arm_calib_file
        )
        try:
    ...
    def _load_hand_pose_action(self, side: str, pose: str) -> dict[str, float]:
        ...
        """
        try:
            from sodexarm.teleoperators.quest_hts.bimanual_calibration import read_yaml
            ...
            logger.warning("Cannot load %s hand %s pose from %s: %s", side, pose, self.config.hand_calib_file, exc)
            return {}
    ...
    def _load_start_action(self, side: str) -> dict[str, float]:
        try:
            from sodexarm.teleoperators.quest_hts.bimanual_calibration import load_start_position
    ...
    def action_features(self) -> dict[str, type]:
        ...
        if self.config.mode != "hand-only":
            keys.update({k: float for k in arm_keys})
        if self.config.mode != "arm-only":
            keys.update({k: float for k in hand_keys})
        return keys

    @property
    def feedback_features(self) -> dict[str, type]:
        return {k: float for k in ARM_KEYS_R + ARM_KEYS_L + HAND_KEYS_R + HAND_KEYS_L}
    ...
    def connect(self, calibrate: bool = True) -> None:
        ...
        if self.config.start_receiver:
            ...
            self._thread = threading.Thread(
                ...
                args=(server,),
                daemon=True,
                name="quest_hts_bimanual_tcp",
            )
            self._thread.start()
            print(
                f"[QuestHTSBimanual] TCP server listening on {self.config.host}:{self.config.port}",
                flush=True,
            )
    ...
    def _await_baseline_capture(self, sides_hint: str) -> None:
        ...
        if countdown <= 0:
            print(
                f"\n[QuestHTSBimanual] Hold {sides_hint} at the robot start pose,"
    ...
    def begin_episode(self, robot=None, events=None) -> None:
        ...
        """
        print("[QuestHTSBimanual] begin_episode: returning to start pose (+ open hands)...", flush=True)
        ...
        if robot is not None:
            ...
            if self.config.move_to_start:
                ...
                obs = robot.get_observation()
                self.send_feedback({k: v for k, v in obs.items() if isinstance(v, (int, float))})
                print("[QuestHTSBimanual] begin_episode: start pose reached.", flush=True)
            else:
                print("[QuestHTSBimanual] begin_episode: move_to_start disabled — skipping move.", flush=True)
        else:
            print("[QuestHTSBimanual] begin_episode: no robot handle — cannot move to start!", flush=True)
        ...
        self._paused = False
        print(f"[QuestHTSBimanual] begin_episode: aligned + baseline re-captured for {ready} — RESUMED.",
              flush=True)
    ...
    def end_episode(self, robot=None) -> None:
        ...
        """
        self._paused = True
        print("[QuestHTSBimanual] end_episode: tracking PAUSED — robot frozen (arm + hands). "
              "Reset the environment freely.", flush=True)
    ...
    def _await_alignment(self, events=None) -> None:
        ...
        """
        print("[QuestHTSBimanual] ALIGN: robot is at START and FROZEN.", flush=True)
        print("  → Reset the environment and bring both hands to match the robot's start pose.", flush=True)
        print("  → Press the → (right arrow) AGAIN to re-calibrate the hands and start the next episode.", flush=True)
    ...
    def prepare_control_session(self, robot=None) -> None:
        ...
        print("[QuestHTSBimanual] Waiting for Quest HTS stream (left + right)...", flush=True)
        print(f"  Start Meta Quest Hand Tracking Streamer → send to {self.config.host}:{self.config.port}", flush=True)
        ...
        print("[QuestHTSBimanual] Quest HTS stream connected (left + right).", flush=True)
        ...
        # (no handedness) and is the recommended default for symmetric bimanual control.
        ...
        print(
            f"[QuestHTSBimanual] IK mode = {mode_str}\n"
        ...
        if self.config.move_to_start and robot is not None:
            print("[QuestHTSBimanual] Moving robot to start positions...", flush=True)
            self._move_to_start(robot)
            print("[QuestHTSBimanual] Start positions reached.", flush=True)
            obs = robot.get_observation()
            self.send_feedback({k: v for k, v in obs.items() if isinstance(v, (int, float))})
                ...
                if state.latest_wrist is not None:
                    ...
                    if self.config.use_elbow_ik:
                        ...
                        if state.start_joint_degrees:
                            ...
                            theta1 = math.radians(state.start_joint_degrees.get("shoulder_lift", 0.0)) - kin.theta1_offset
                            state.start_elbow_robot = np.array([kin.l1 * math.cos(theta1), kin.l1 * math.sin(theta1)])
        ...
        missing = [
            s for s, st in (("right", self._right), ("left", self._left))
        ...
        if missing:
            ...
            print(
                f"[QuestHTSBimanual] Warning: baseline missing for {missing} — continuing with {captured} only.",
                flush=True,
            )
        print("[QuestHTSBimanual] Wrist baselines captured. Teleoperation started.", flush=True)
    ...
    def _move_to_start(self, robot) -> None:
        ...
        if not goal:
            print("[QuestHTSBimanual] No start positions saved; skipping move-to-start.", flush=True)
            return
    ...
    def _apply_pitch_hold(self, state: _SideState, ik_joints: dict, shoulder_lift_deg: float, elbow_flex_deg: float) -> None:
    ...
    def _build_arm_action_locked(self, state: _SideState) -> dict[str, float]:
        ...
        """
        fallback = dict(state.last_valid_ik_action) if state.last_valid_ik_action else dict(state.start_action)
        ...
        # Stale check
        if (state.latest_wrist_s is not None
            ...
            if not getattr(state, "_stale_warned", False):
                print(
                    f"[QuestHTSBimanual] {state.side} wrist data stale ({age:.1f}s) — "
        ...
        if self.config.use_full_ik and state.start_tcp_pose is not None:
            ...
            R_delta_robot = _R_QUEST_TO_ROBOT @ human_delta[:3, :3] @ _R_QUEST_TO_ROBOT.T
            R_delta_scaled = _scale_rotation(R_delta_robot, ori_scale)
            target_R = state.start_tcp_pose[:3, :3] @ R_delta_scaled

            target_tcp = np.eye(4, dtype=float)
            target_tcp[:3, :3] = target_R
        ...
        elif self.config.use_elbow_ik and state.elbow_baseline_pos_quest is not None and state.start_elbow_robot is not None:
            ...
            target_x, target_y = _project_to_reach(
                target_x, target_y, l1, l2, self.config.max_reach_fraction
            )
        ...
        # matches intent (fixes the "ガコっ" drop at hand-control transition without raising P).
        ...
        reference = state.last_controlled_action or {
            k: float(state.start_action.get(k, arm_action[k]))
            for k in controlled_action
        }
    ...
    def _build_hand_action_locked(self, state: _SideState) -> dict[str, float]:
        ...
        # Check stale
        if (state.latest_landmarks_s is not None
    ...
    def handle_hts_line(self, line: str) -> bool:
        ...
        # Arm-only correction: the Quest app labels "wrist" and "landmarks" lines independently, so
        # only the wrist (arm) channel can come in mislabeled while landmarks (hand) stays correct.
        # See config_quest_hts_bimanual.swap_arm_side for the full rationale.
        if kind == "wrist" and self.config.swap_arm_side:
            side = "left" if side == "right" else "right"
        now = time.monotonic()
        with self._lock:
            ...
            if kind == "wrist":
                ...
                if needs_baseline or was_stale:
                    ...
                    if needs_baseline:
                        print(
                            f"[QuestHTSBimanual] Late baseline captured for {side} — arm now active.",
                            flush=True,
                        )
                    else:
                        print(
                            f"[QuestHTSBimanual] {side} tracking recovered — baseline reset, resuming.",
                            flush=True,
                        )
            elif kind == "landmarks":
        ...
        try:
            while not self._stop_event.is_set():
                try:
                    ...
                    break
                print(f"[QuestHTSBimanual] Quest HTS client connected from {addr}", flush=True)
                threading.Thread(
        ...
        with conn:
            ...
            while not self._stop_event.is_set():
                try:
                    ...
                    break
                if not data:
                    print("[QuestHTSBimanual] Quest HTS client disconnected.", flush=True)
                    break
                try:
