...
class QuestHTSRightTeleoperatorConfig(TeleoperatorConfig):
    ...
    side_mapping_file: Path = Path("configs/quest_hts_physical_side_mapping.yaml")
    ...
    tcp_config_file: Path = Path("configs/quest_hts_right_amazinghand_tcp.yaml")
    ...
    home_tcp_pose_file: Path = Path("configs/quest_hts_right_so101_amazinghand_home_tcp.yaml")
    ...
    right_hand_calibration_file: Path = Path("configs/quest_hts_right_hand_calibration.yaml")
    ...
    def __post_init__(self) -> None:
        ...
        if self.input_assignment_mode not in {"native_right", "physical_side_mapping", "single_visible_right"}:
            raise ValueError(
        ...
        if self.input_assignment_mode == "native_right" and self.physical_hand != "right":
            raise ValueError("--teleop.input-assignment-mode=native_right requires --teleop.physical-hand=right.")
        if self.input_assignment_mode == "single_visible_right" and self.physical_hand != "right":
            raise ValueError("--teleop.input-assignment-mode=single_visible_right requires --teleop.physical-hand=right.")
        ...
        if self.input_assignment_mode == "native_right" and self.input_hand not in {None, "right"}:
            raise ValueError("--teleop.input-assignment-mode=native_right only supports --teleop.input-hand=right.")
        if (
        ...
        if self.arm_control_mode not in {"ik_ee", "joint_delta", "constrained_planar_ik"}:
            raise ValueError("--teleop.arm-control-mode must be ik_ee, joint_delta, or constrained_planar_ik.")
