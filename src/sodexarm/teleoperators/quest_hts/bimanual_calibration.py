#!/usr/bin/env python
"""Bimanual calibration and startup helpers (originally scripts/quest_hts/phase31).
...
Hand calibration is saved to configs/quest_hts_bimanual_hand_calibration.yaml
Startup positions are saved to configs/quest_hts_bimanual_startup.yaml

Usage example:
  python -m sodexarm.teleoperators.quest_hts.bimanual_calibration capture-hand --side right --pose fist
  python -m sodexarm.teleoperators.quest_hts.bimanual_calibration capture-hand --side right --pose mid
  python -m sodexarm.teleoperators.quest_hts.bimanual_calibration capture-hand --side right --pose open
  python -m sodexarm.teleoperators.quest_hts.bimanual_calibration capture-startup --side right
  python -m sodexarm.teleoperators.quest_hts.bimanual_calibration startup --side right
"""
...
DEFAULT_HAND_CALIB_FILE = Path("configs/quest_hts_bimanual_hand_calibration.yaml")
DEFAULT_STARTUP_FILE = Path("configs/quest_hts_bimanual_startup.yaml")
...
DEFAULT_ARM_PORTS = {
    "right": "/dev/ttyso101_amazinghand_l_arm",  # physical right, symlink=l_arm
    "left": "/dev/ttyso101_amazinghand_r_arm",   # physical left, symlink=r_arm
}
DEFAULT_HAND_PORTS = {
...
def all_bimanual_keys() -> tuple[str, ...]:
...
# Default human-hand feature used for new calibrations / runtime (2026-06-19).
...
def hand_feature_vector_from_landmarks(
    ...
    The human pinky (little) is intentionally NOT tracked (2026-06-16) — the AmazingHand has
    ...
    - ``"curl"`` (default, 2026-06-19): sum of the three joint bend angles along the finger
...
class PiecewiseHandMapper:
    ...
    # Fixed human-finger → robot motor-pair binding (2026-06-16).
...
def build_hand_mapping(
    *,
    ...
    mapping["_feature_mode"] = side_data.get(
        "feature_mode", payload.get("feature_mode", "distance")
    )
    # Must match PiecewiseHandMapper.FINGER_GROUPS (fixed binding, 2026-06-16):
    ...
    try:
        ...
        with conn:
            while time.monotonic() < deadline and len(lines) < max_lines:
                ...
                ready = all(
                    summary.get(f"has_{s}_wrist") and summary.get(f"has_{s}_landmarks")
                    for s in sides
                )
                if ready:
                    break
...
def interpolate_joint_steps(
    ...
    *,
    ...
    for i in range(1, steps + 1):
        t = i / steps
        waypoint = {
            k: start.get(k, goal[k]) + t * (goal[k] - start.get(k, goal[k]))
            for k in keys
        }
...
def require_hardware_motion_flags(require_hardware: bool, enable_motion: bool) -> None:
    if require_hardware and not enable_motion:
        raise RuntimeError(
            "Hardware motion requires --enable-motion flag. "
            "Pass --enable-motion to confirm motion is safe."
        )
...
def run_startup_sequence(
    ...
    *,
    ...
    if not ready:
        missing = [
            key for key in _required_for_startup(config.sides)
    ...
    if config.enable_motion or config.require_hardware:
        ...
        for side in config.sides:
            ...
            make_adapter = arm_adapter_factory or (lambda cfg=cfg: ArmAdapter(
...
# --------------------------------------------------------------------------- #
# Bimanual record/teleop command builders
# --------------------------------------------------------------------------- #
...
def build_bimanual_record_command(
    ...
    *,
    ...
    """Build lerobot record command for bimanual SO-101 + AmazingHand."""
    return _command(
        "lerobot.scripts.control_robot",
        "record",
        "--robot.type", "bi_so101_amazinghand",
        "--robot.right-arm-port", port_map.right_arm_port,
        "--robot.left-arm-port", port_map.left_arm_port,
        "--robot.right-hand-port", port_map.right_hand_port,
        "--robot.left-hand-port", port_map.left_hand_port,
        "--teleop.type", "quest_hts_right",
        "--teleop.input-assignment-mode", "native_right",
        "--teleop.host", hts_host,
        "--teleop.port", str(hts_port),
        "--repo-id", repo_id,
        "--num-episodes", str(num_episodes),
        "--episode-time-s", str(episode_time_s),
        "--reset-time-s", str(reset_time_s),
        "--push-to-hub", "false",
    )


def build_bimanual_teleop_command(
    port_map: PhysicalPortMap,
    *,
    hts_host: str = DEFAULT_HTS_HOST,
    hts_port: int = DEFAULT_HTS_PORT,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Phase31: Bimanual calibration and startup.",
    )
    ...
    p_cs.add_argument("--from-hardware", action="store_true", help="Read current arm positions from hardware.")
    ...
    p_startup.add_argument("--enable-motion", action="store_true", help="Allow robot to move to start position.")
