...
2. Run:  python -m sodexarm.teleoperators.quest_hts.axis_map_diagnostic
...
def streaming_readout(sh: Shared, period_s: float) -> None:
    ...
    try:
        while not sh.stop.is_set():
            ...
            print(f"[{now:8.1f}] lines total={c['total']} wrist={c['wrist']} "
                  f"landmarks={c['landmarks']} unparsed={c['none']}")
            if c["wrist"] == 0:
                for raw in raws:
                    print(f"    raw: {raw[:90]}")
                if c["total"] == 0:
                    print("    >>> connection made but 0 bytes — restart the Quest streamer "
    ...
    try:
        for side in ("right", "left"):
            ...
            for s, motion, robot_axis in _CAPTURE_PLAN:
                ...
                print(f"    → Δ=({delta[0]:+.3f},{delta[1]:+.3f},{delta[2]:+.3f})  "
                      f"dominant {dom}  |Δ|={mag:.3f}m"
                      + ("   ⚠ very small move, redo" if mag < 0.05 else ""))
...
def _report_capture(results: dict[tuple[str, str], tuple[float, float, float]]) -> None:
    ...
    for side in ("right", "left"):
        ...
        need = {"forward": results.get((side, "forward")),
