...
from sodexarm.model.so101_kinematics import SO101Kinematics, SO101LeRobotCalibration
...
DEFAULT_SIDE_MAPPING_FILE = Path("configs/quest_hts_physical_side_mapping.yaml")
...
def default_start_pose_file(side: str) -> Path:
    if side == "both":
        return Path("configs/quest_hts_start_pose_bimanual.yaml")
    return Path(f"configs/quest_hts_start_pose_{side}.yaml")


def default_constraints_file(side: str) -> Path:
    if side == "both":
        return Path("configs/quest_hts_bimanual_so101_constraints.yaml")
    return Path(f"configs/quest_hts_{side}_so101_constraints.yaml")
...
def default_hand_calibration_file(side: str) -> Path:
    return Path(f"configs/quest_hts_{side}_hand_calibration.yaml")
...
def default_robot_port(side: str) -> str:
    return "/dev/ttyso101_amazinghand_l_arm" if side == "right" else "/dev/ttyso101_amazinghand_r_arm"
...
def lookup_action_value(action: Mapping[str, object], key: str, side: str, default: float | None = None) -> float | None:
...
def load_side_calibration(path: Path | str | None, side: str, *, action_units: str = "range_m100_100") -> SO101LeRobotCalibration:
    ...
    if any(key.startswith(side_prefix(side)) for key in data):
        data = {strip_side_prefix(key): value for key, value in data.items() if key.startswith(side_prefix(side))}
...
def wrist_to_payload(wrist: Sequence[float] | None) -> tuple[dict[str, float | None], dict[str, float | None]]:
    if wrist is None:
        return (
    ...
    *,
    ...
    payload = {
        ...
        "robot": {
            ...
            "fixed_joint_start_action": {
                ...
                for key in DEFAULT_FIXED_JOINTS
            },
    ...
    if include_transport_debug:
        payload["transport_debug"] = {
            f"raw_label_for_{side}": transport_raw_label if transport_raw_label is not None else bound_input_label,
        }
    return payload
...
def _limits_for_keys(calibration: SO101LeRobotCalibration, side: str, keys: Sequence[str]) -> dict[str, dict[str, float]]:
