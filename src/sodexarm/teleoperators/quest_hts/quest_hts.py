...
from sodexarm.teleoperators.quest_hts.bimanual_mapping import (
    BIMANUAL_ACTION_KEYS,
    LEROBOT_BIMANUAL_ACTION_KEYS,
    BimanualDryRunConfig,
    BimanualDryRunMapper,
    SideConfig,
    format_result,
    provisional_to_lerobot_action,
)
from sodexarm.teleoperators.quest_hts.hand_calibration import (
...
from sodexarm.teleoperators.quest_hts.planar_ik import (
...
from sodexarm.teleoperators.quest_hts.right_arm_mapping import RightArmMappingConfig
from sodexarm.teleoperators.quest_hts.right_hand_mapping import RightHandMappingConfig

from .config_quest_hts import QuestHTSRightTeleoperatorConfig, QuestHTSTeleoperatorConfig

logger = logging.getLogger(__name__)

RIGHT_ARM_KEYS = ("shoulder_pan.pos", "shoulder_lift.pos", "elbow_flex.pos", "wrist_flex.pos", "wrist_roll.pos")
...
class QuestHTSTeleoperator(Teleoperator):
    ...
    This class consumes HTS TCP CSV lines and returns LeRobot bimanual
    ...
    def __init__(self, config: QuestHTSTeleoperatorConfig):
        ...
        self.config = config
        self._mapper = BimanualDryRunMapper(_make_mapper_config(config))
    ...
    def action_features(self) -> dict[str, type]:
        return {key: float for key in LEROBOT_BIMANUAL_ACTION_KEYS}
...
class QuestHTSRightTeleoperator(Teleoperator):
    ...
    def __init__(self, config: QuestHTSRightTeleoperatorConfig):
        ...
        self._mapper = BimanualDryRunMapper()
        ...
        self._live_side_confirmed = True if self._is_native_right_mode() else not config.require_live_side_confirmation
        ...
        self._smoothed_arm_delta = {key: 0.0 for key in RIGHT_ARM_KEYS}
    ...
    @cached_property
    def action_features(self) -> dict[str, type]:
        return {key: float for key in self._mode_keys(canonical=True)}

    @cached_property
    def feedback_features(self) -> dict[str, type]:
        return {key: float for key in RIGHT_CANONICAL_ACTION_KEYS}
    ...
    def _initial_bound_input_label(config: QuestHTSRightTeleoperatorConfig) -> str | None:
        ...
        if config.binding_mode == "saved_map" and Path(config.side_mapping_file).exists():
            return raw_label_for_physical_side(load_side_mapping(config.side_mapping_file), config.physical_hand)
        return None
    ...
    def prepare_control_session(self, robot=None) -> None:
        ...
        if self.config.print_current_tcp_pose or self.config.save_current_tcp_as_home:
            ...
            if current_tcp is not None:
                ...
                if self.config.save_current_tcp_as_home:
                    ...
                    self._emit("Saved current right SO-101 AmazingHand TCP home pose to %s", self.config.home_tcp_pose_file, force_print=True)
        ...
        if self._is_constrained_planar_mode():
            ...
            if not wrist_ready:
                raise RuntimeError(
                    "Cannot capture constrained planar IK baseline: "
                    "timed out waiting for fresh wrist pose."
                )
            with self._lock:
                if self._latest_wrist is None:
                    raise RuntimeError("Cannot capture constrained planar IK baseline: no fresh bound wrist pose received.")
                if not self._capture_constrained_planar_baseline_locked(self._latest_wrist):
                    raise RuntimeError("Cannot capture constrained planar IK baseline: start pose file is incomplete.")
        ...
        if not wrist_ready:
            raise RuntimeError(
                "Cannot capture QuestHTS end-effector baseline: "
                "timed out waiting for fresh wrist pose."
            )
        with self._lock:
            if self._latest_wrist is None:
                raise RuntimeError("Cannot capture QuestHTS end-effector baseline: no fresh bound wrist pose received.")
            if not self._capture_ee_baseline_locked(self._latest_wrist):
                raise RuntimeError("Cannot capture QuestHTS end-effector baseline: robot arm feedback is incomplete.")
    ...
    def get_episode_metadata(self) -> dict[str, object]:
        metadata = {
            ...
            "start_pose_file": None if self.config.start_pose_file is None else str(self.config.start_pose_file),
            "constraints_file": None if self.config.constraints_file is None else str(self.config.constraints_file),
            ...
            "controlled_joints": [prefixed_key(self._robot_side, key) for key in self._planar_controlled_joints],
            ...
            "target_tcp_pose": None if self._last_target_tcp_pose is None else pose_to_dict(self._last_target_tcp_pose),
    ...
    def connect(self, calibrate: bool = True) -> None:
        ...
        if self.config.require_calibration and self.config.mode != "arm-only" and not self._hand_calibration:
            raise RuntimeError(f"QuestHTS hand calibration missing: {self.config.right_hand_calibration_file}")
        if (
    ...
    def send_feedback(self, feedback: dict[str, Any]) -> None:
        with self._lock:
            ...
            numeric = {key: float(value) for key, value in normalized_feedback.items() if key in RIGHT_ACTION_KEYS}
    ...
    def _update_raw_input_locked(self, side: str, kind: str, values: tuple[float, ...], now: float) -> None:
        ...
        if kind == "wrist":
            ...
            raw["span_m"] = _wrist_translation_distance(first_wrist, values) if first_wrist is not None else 0.0
        elif kind == "landmarks":
    ...
    def _maybe_auto_bind_locked(self) -> None:
        if self._bound_input_label is not None or self.config.binding_mode != "auto" or self.config.manual_baseline:
            return
    ...
    def _set_bound_input_label_locked(self, label: str, confidence: float, stats: dict[str, object]) -> None:
        ...
        if self.config.print_hand_binding:
            ...
            if self.config.print_debug_transport_labels:
                self._emit("transport_debug.raw_label_for_%s=%s", self.config.physical_hand, label, force_print=True)
    ...
    def _run_binding_step(self) -> None:
        if self._is_native_right_mode():
            with self._lock:
                self._set_bound_input_label_locked("right", 1.0, {"right": _binding_side_stats(self._raw_inputs["right"])})
                self._live_side_confirmed = True
            return
        if self._is_single_visible_right_mode():
            if not self.assign_single_visible_right_input():
        ...
        if self.config.binding_mode == "saved_map":
            label = raw_label_for_physical_side(load_side_mapping(self.config.side_mapping_file), self.config.physical_hand)
            with self._lock:
                self._set_bound_input_label_locked(label, 1.0, {label: _binding_side_stats(self._raw_inputs[label])})
            return
        if self.config.binding_mode == "explicit":
    ...
    def _build_joint_delta_arm_action_locked(self) -> tuple[dict[str, float], dict]:
        ...
        for index, key in enumerate(RIGHT_ARM_KEYS):
            limited_delta = max(-self.config.max_arm_cumulative, min(self.config.max_arm_cumulative, delta[key]))
            cumulative_limited = cumulative_limited or abs(limited_delta - delta[key]) > 1e-9
            alpha = self.config.arm_smoothing_alpha
            self._smoothed_arm_delta[key] = alpha * limited_delta + (1.0 - alpha) * self._smoothed_arm_delta[key]
            lo, hi = self.config.arm_joint_limits[index]
            target_before_step[key] = max(lo, min(hi, self._neutral_action[key] + self._smoothed_arm_delta[key]))
    ...
    def _capture_constrained_planar_baseline_locked(self, wrist: tuple[float, ...]) -> bool:
        ...
        self._hand_start_action = {
            key: float(val)
            for key, val in self._last_feedback.items()
    ...
    def _clamp_planar_action_locked(self, action: Mapping[str, float]) -> tuple[dict[str, float], dict[str, tuple[float, float, float]]]:
    ...
    def _build_constrained_planar_ik_arm_action_locked(self) -> tuple[dict[str, float], dict]:
        ...
        if reference is None:
            reference = {k: float(self._planar_start_action[k]) for k in controlled_keys if k in self._planar_start_action}
        ...
        self._last_controlled_joint_action = {k: action_after_limit[k] for k in controlled_keys if k in action_after_limit}
        debug = {
            "arm_baseline_ready": True,
            "baseline_ready": True,
            "arm_target_source": "fresh",
    ...
    def _build_ik_ee_arm_action_locked(self) -> tuple[dict[str, float], dict]:
        if not self._ee_baseline_captured or self._human_wrist_baseline_pose is None or self._robot_tcp_baseline_pose is None:
            return {}, {
                "arm_baseline_ready": False,
                "baseline_ready": False,
                "arm_target_source": "none",
    ...
    def _map_human_delta_to_robot_delta(self, human_delta: np.ndarray) -> tuple[np.ndarray, dict[str, object]]:
        ...
        mapped[:3, :3] = rotation_matrix_from_rpy(float(mapped_rpy[0]), float(mapped_rpy[1]), float(mapped_rpy[2]))
        return mapped, {
    ...
    def _receiver_debug_locked(self) -> dict[str, object]:
        ...
        debug = {
            ...
            "receiver_thread_alive": self._receiver_alive and self._thread is not None and self._thread.is_alive(),
    ...
    def _build_hand_action_locked(self) -> tuple[dict[str, float], dict]:
        ...
        if input_label is None:
            return {}, {"hand_target_source": "none", "calibration_target_used": False, "action_block_reason": "binding_not_completed"}
        ...
        if age is not None and age > self._stale_timeout_s():
            return {}, {"hand_target_source": "none", "calibration_target_used": False, "latest_landmarks_age_s": age}
        ...
        if first_hand_action and self._hand_start_action:
            for key in list(hand_targets.keys()):
                ...
                if abs(hand_targets[key] - start_val) > max_hand_delta_from_start:
                    ...
                    hand_targets[key] = start_val + max_hand_delta_from_start * (1.0 if overshoot > 0 else -1.0)
                ...
                while not self._stop_event.is_set():
                    try:
                        ...
                        with self._lock:
                            ...
                            if not (
                                intentional_close
                                and self._is_bad_file_descriptor_error(exc)
...
def _make_mapper_config(config: QuestHTSTeleoperatorConfig) -> BimanualDryRunConfig:
    return BimanualDryRunConfig(
        right=SideConfig(
...
def _project_planar_target_to_reach(x: float, y: float, l1: float, l2: float) -> tuple[float, float, bool, str]:
...
def _format_action(action: dict[str, float]) -> str:
    ordered = OrderedDict((key, action[key]) for key in LEROBOT_BIMANUAL_ACTION_KEYS)
    return "{" + ", ".join(f"{key}: {value:+.3f}" for key, value in ordered.items()) + "}"


def _format_combined_action(action: dict[str, float]) -> str:
    ordered = OrderedDict((key, action[key]) for key in BIMANUAL_ACTION_KEYS)
    return "{" + ", ".join(f"{key}: {value:+.3f}" for key, value in ordered.items()) + "}"
