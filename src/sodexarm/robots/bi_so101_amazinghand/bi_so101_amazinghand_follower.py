...
"""Bilateral SO-101 + AmazingHand follower (26 DOF).
...
Each arm's all motors (STS3215 IDs 1-5 + SCS0009 IDs 1-8) share ONE physical RS485 port.
"""
...
ALL_BI_MOTORS = R_MOTORS + L_MOTORS  # 26 total


class BiSO101AmazingHandFollower(Robot):
    """26-DOF bilateral follower (right + left SO-101 + AmazingHand)."""

    config_class = BiSO101AmazingHandFollowerConfig
    name = "bi_so101_amazinghand_follower"

    def __init__(self, config: BiSO101AmazingHandFollowerConfig):
        ...
        # live on ONE RS485 bus. They must (a) use non-colliding IDs (right=arm_ids_r 21-25,
        # left=arm_ids_l 11-15) and (b) share ONE PortHandler — opening the same /dev twice causes
        ...
        # Arm motor IDs come from config regardless of bus topology. The right arm's motors are
        # physically renumbered to arm_ids_r (21-25) and keep those IDs whether they share the left
        # arm's bus (single-port) or sit on their own adapter (two-port). Only PortHandler sharing
        # depends on single-port mode — and only matters when BOTH arms are active.
        ...
        if self._single_arm_port:
            ...
            logger.info(
                ...
                config.port_r, r_arm_ids, l_arm_ids,
            )
        else:
            ...
            if want_r and want_l:
                logger.info(
                    ...
                    config.port_r, r_arm_ids, config.port_l, l_arm_ids,
                )
            else:
        ...
        def _sub_cfg(port, hand_port, arm_ids, suffix):
            return SO101AmazingHandFollowerConfig(
                ...
                id=f"{config.id}_{suffix}" if config.id else suffix,
                ...
                invert_hand_id8=(
                    config.right_hand_id8_inverted if suffix == "r" else config.left_hand_id8_inverted
                ),
                placefruit_right_compat=(config.right_hand_placefruit_compat if suffix == "r" else False),
                placefruit_left_grip_gain=(
                    config.left_hand_placefruit_grip_gain if suffix == "l" else 1.0
                ),
            )
    ...
    def is_connected(self) -> bool:
        return (
            all(arm.is_connected for _, arm in self._arms())
    ...
    def connect(self, calibrate: bool = True) -> None:
        ...
        # bi-level calibration file so subsequent runs skip per-arm calibration entirely.
