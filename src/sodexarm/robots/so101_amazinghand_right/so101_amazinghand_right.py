...
def _canonical_motor_key(motor: str) -> str:
    return f"r_{motor}.pos"


def _motor_name_from_action_key(key: str) -> str | None:
    if not key.endswith(".pos"):
        return None
    motor = key.removesuffix(".pos")
    if motor.startswith("r_"):
        return motor[2:]
    if motor.startswith("l_"):
        return None
    return motor
...
def _hand_yaml_key(motor: str) -> str:
    return f"r_{motor}.pos"
...
def _missing_quest_hand_features(calibration: dict, hand_motors: list[str] | tuple[str, ...] = HAND_MOTORS) -> list[str]:
    missing: list[str] = []
    human_poses = calibration.get("human_poses", {}) if isinstance(calibration.get("human_poses", {}), dict) else {}
    ...
    for pose in ("open", "closed"):
        features = human_poses.get(pose, {}).get("features", {}) if isinstance(human_poses.get(pose, {}), dict) else {}
        if not features:
...
def _quest_hand_motor_calibration(
    ...
    robot = calibration.get("robot", {}) if isinstance(calibration.get("robot", {}), dict) else {}
    safe_min = robot.get("safe_min_raw_positions", {}) if isinstance(robot.get("safe_min_raw_positions", {}), dict) else {}
    safe_max = robot.get("safe_max_raw_positions", {}) if isinstance(robot.get("safe_max_raw_positions", {}), dict) else {}
    open_raw = robot.get("open_raw_positions", {}) if isinstance(robot.get("open_raw_positions", {}), dict) else {}
    closed_raw = robot.get("closed_raw_positions", {}) if isinstance(robot.get("closed_raw_positions", {}), dict) else {}
...
class SO101AmazingHandRight(SO101AmazingHandFollower):
    ...
    def __init__(self, config: SO101AmazingHandRightConfig):
        ...
        if self.config.hand_calibration_source == "quest_yaml" and self._quest_hand_calibration:
            missing = _missing_quest_hand_features(self._quest_hand_calibration)
            if not missing:
    ...
    def observation_features(self) -> dict[str, type | tuple]:
        feats: dict = {_canonical_motor_key(m): float for m in _mode_motors(self.config.mode)}
    ...
    def action_features(self) -> dict[str, type]:
        return {_canonical_motor_key(m): float for m in _mode_motors(self.config.mode)}
    ...
    def calibration_status(self) -> dict[str, object]:
        ...
        missing_arm = [motor for motor in ARM_MOTORS if _needs_arm_calibration(self.config.mode) and motor not in self.calibration]
        missing_hand = [
            motor
            for motor in HAND_MOTORS
        ...
        if _needs_hand_calibration(self.config.mode):
            if self.config.hand_calibration_source == "quest_yaml":
                if not self._quest_hand_calibration:
                    ...
                    missing_hand_features = _missing_quest_hand_features(self._quest_hand_calibration)
    ...
    def _require_existing_calibration(self) -> None:
        ...
        if self.config.hand_calibration_source == "quest_yaml" and _needs_hand_calibration(self.config.mode):
            if status["missing_hand_features"]:
                raise RuntimeError(
    ...
    def _calibrate_arm_only(self) -> None:
        ...
        print(self._prompt("Move all arm joints (except wrist_roll) through full range. Press ENTER to stop ..."))
    ...
    def _calibrate_hand_only(self) -> None:
        ...
        if too_small:
            raise ValueError(f"Some hand motors have a calibration span smaller than {_HAND_CAL_MIN_SPAN}: {too_small}")
    ...
    def _is_fatal_hand_error(text: str) -> bool:
        return (
            "Overload error" in text
            or "Input voltage error" in text
            or "There is no status packet" in text
        )
    ...
    def _latch_hand_fault(self, motor: str, exc: Exception) -> None:
        ...
        if self.hand_fault_first_timestamp is None:
            ...
            self.hand_fault_key = _canonical_motor_key(motor)
            self.hand_fault_type = fault_type
        self.right_hand_disabled = True
        message = (
            f"hand fault ({fault_type}) on motor={motor!r} id={motor_id}; "
            "disabling hand I/O for this run"
        )
    ...
    def get_observation(self) -> RobotObservation:
        obs: dict = {}
        if self.config.mode != "arm-only":
            if not self.right_hand_disabled:
                with _big_endian():
                    for motor in self.hand_bus.motors:
                        if motor not in HAND_MOTORS:
                            continue
                        try:
                            obs[_canonical_motor_key(motor)] = self.hand_bus.read("Present_Position", motor)
                            ...
                            obs[_canonical_motor_key(motor)] = 0.0
            ...
            try:
                ...
                for motor in self.arm_bus.motors:
                    try:
                        ...
                        arm_pos[motor] = 0.0
            obs.update({_canonical_motor_key(k): v for k, v in arm_pos.items() if k in ARM_MOTORS})
    ...
    def _handle_send_error(self, target: str, exc: Exception) -> None:
        ...
        if target == "hand":
            ...
            message = f"right hand send failed; policy={policy}: {text}"
            self.last_hand_error = message
            self.hand_error_history.append(message)
            ...
            message = f"right arm send failed; policy={policy}: {text}"
        ...
        with _big_endian():
            for motor, motor_cfg in self.hand_bus.motors.items():
                ...
                try:
                    ...
                    logger.warning("Hand shutdown torque disable error detail for motor ID %s: %s", motor_id, exc)

    @check_if_not_connected
    def send_action(self, action: RobotAction) -> RobotAction:
        goal_pos = {}
        for key, value in action.items():
            motor = _motor_name_from_action_key(key)
        ...
        if arm_goal and not self.right_arm_disabled:
            try:
                ...
                sent.update({_canonical_motor_key(k): v for k, v in arm_goal.items()})
            except Exception as exc:  # noqa: BLE001
        ...
        if hand_goal and not self.right_hand_disabled:
            try:
                ...
                self._clear_after_hand_bus()
                sent.update({_canonical_motor_key(k): v for k, v in hand_goal.items()})
            except Exception as exc:  # noqa: BLE001
