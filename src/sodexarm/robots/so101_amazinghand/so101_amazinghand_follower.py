...
# PlaceFruit was recorded before right ID3's raw range was expanded from [200, 800] to
# [200, 1013]. Convert between the old policy coordinate and the current bus normalization.
_PLACEFRUIT_RIGHT_ID3_ACTION_SCALE = (800 - 200) / (1013 - 200)

# Current measured open/fist anchors. Clamping these two joints prevents the old ACT policy from
# driving the thumb/index past poses that are meaningful on the current right hand.
_PLACEFRUIT_RIGHT_ACTION_LIMITS = {
    "finger1_motor1": (0.0, 98.16666666666667),
    "finger1_motor2": (0.33333333333333337, 71.5),
    "finger2_motor2": (2.666666666666667, 97.16666666666667),
}

_PLACEFRUIT_LEFT_OPEN_FIST = {
    "finger1_motor1": (100.0, 0.0),
    "finger1_motor2": (16.0, 59.0),
    "finger2_motor1": (75.66666666666667, 0.0),
    "finger2_motor2": (18.833333333333332, 100.0),
    "finger3_motor1": (85.16666666666667, 0.0),
    "finger3_motor2": (21.333333333333336, 100.0),
    "finger4_motor1": (92.33333333333333, 0.0),
    "finger4_motor2": (10.333333333333334, 100.0),
}
...
class SO101AmazingHandFollower(Robot):
    ...
    def __init__(
        ...
        *,
        ...
        #   port on disconnect (the owner — the bimanual follower — closes it once, after BOTH
        #   arms have had their torque disabled).
        super().__init__(config)
        self.config = config
        if config.placefruit_left_grip_gain <= 0:
            raise ValueError("placefruit_left_grip_gain must be greater than 0")
        ...
        self._port_handler = (
            arm_port_handler if arm_port_handler is not None
            else _make_shared_port_handler(config.port)
        )
        ...
        # causes packet collisions and juddery motion. (Seen 2026-06-09 after a reboot shuffled
        # the ACM numbers so a raw /dev/ttyACMx hand_port landed on the arm's device.)
        ...
        self._hand_port_handler = (
            _make_shared_port_handler(_hand_port)
            if self._separate_hand_port
            else self._port_handler
        )
        ...
        self.arm_bus = FeetechMotorsBus(
            port=config.port,
            motors={
                name: Motor(arm_ids[i], "sts3215", norm_arm)
    ...
    def _logical_hand_value(self, motor: str, value: float) -> float:
        """Map physical hand readback into the logical policy coordinate."""
        value = float(value)
        if self.config.invert_hand_id8 and motor == "finger4_motor2":
            value = 100.0 - value
        if self.config.placefruit_right_compat and motor == "finger2_motor1":
            value = value / _PLACEFRUIT_RIGHT_ID3_ACTION_SCALE
            value = max(0.0, min(100.0, value))
        if self.config.placefruit_left_grip_gain != 1.0 and motor in _PLACEFRUIT_LEFT_OPEN_FIST:
            open_value, fist_value = _PLACEFRUIT_LEFT_OPEN_FIST[motor]
            physical_closedness = (value - open_value) / (fist_value - open_value)
            policy_closedness = physical_closedness / self.config.placefruit_left_grip_gain
            policy_closedness = max(0.0, min(1.0, policy_closedness))
            value = open_value + policy_closedness * (fist_value - open_value)
        return value

    def _physical_hand_goal(self, motor: str, value: float) -> float:
        """Map logical policy action into the physical command coordinate."""
        value = float(value)
        if self.config.placefruit_right_compat:
            if motor == "finger2_motor1":
                value = max(0.0, min(100.0, value)) * _PLACEFRUIT_RIGHT_ID3_ACTION_SCALE
            elif motor in _PLACEFRUIT_RIGHT_ACTION_LIMITS:
                low, high = _PLACEFRUIT_RIGHT_ACTION_LIMITS[motor]
                value = max(low, min(high, value))
        if self.config.placefruit_left_grip_gain != 1.0 and motor in _PLACEFRUIT_LEFT_OPEN_FIST:
            open_value, fist_value = _PLACEFRUIT_LEFT_OPEN_FIST[motor]
            policy_closedness = (value - open_value) / (fist_value - open_value)
            physical_closedness = policy_closedness * self.config.placefruit_left_grip_gain
            physical_closedness = max(0.0, min(1.0, physical_closedness))
            value = open_value + physical_closedness * (fist_value - open_value)
        if self.config.invert_hand_id8 and motor == "finger4_motor2":
            value = 100.0 - value
        return value
    ...
    def _arm_label(self) -> str:
        ...
        Returns 'RIGHT' / 'LEFT' when the arm id ends with '_r' / '_l' (bilateral mode),
    ...
    def _prompt(self, msg: str) -> str:
        """Prefix *msg* with the arm side label (bilateral) or return *msg* unchanged (single)."""
    ...
    def _auto_calibrate_hand(self) -> dict:
        ...
        """
        print(self._prompt(
            "Auto-calibrating AmazingHand — ensure all fingers are free to move fully."
        ))
        ...
        with _big_endian():
            for motor in self.hand_bus.motors:
                lo_pos[motor] = int(
                    self.hand_bus.read("Present_Position", motor, normalize=False)
                )
    ...
    def configure(self) -> None:
        ...
        with self.arm_bus.torque_disabled():
            ...
            for motor in self.arm_bus.motors:
                p_coef = (
                    self.config.arm_p_coefficient_lift if motor in lift_joints
        ...
        try:
            ...
            for motor in self.arm_bus.motors:
                try:
                    present[motor] = self.arm_bus.read("Present_Position", motor, num_retry=3, normalize=False)
                except Exception:
                    present = None
                    break
        if present:
        ...
        with _big_endian():
            ...
            # with Lock=0 before writing. P_Coefficient / Max_Torque_Limit are normally already at
            # their max on this rig (32 / 1000) so writing them is a no-op; the values that move
            # the needle on "grips then drops partway" are the overload-protection pair, which let
    ...
    def get_observation(self) -> RobotObservation:
        ...
        with _big_endian():
            for motor in self.hand_bus.motors:
                key = f"{motor}.pos"
                try:
                    val = self._logical_hand_value(motor, self.hand_bus.read("Present_Position", motor))
                    obs[key] = val
    ...
    def send_action(self, action: RobotAction) -> RobotAction:
        ...
        logical_hand_goal = dict(hand_goal)
        if hand_goal:
            hand_goal = {k: self._physical_hand_goal(k, v) for k, v in hand_goal.items()}
        ...
        return {f"{k}.pos": v for k, v in {**arm_goal, **logical_hand_goal}.items()}
    ...
    def disconnect(self) -> None:
        ...
        # Only close the arm port if WE own it. When two arms share one bus (single-port bimanual),
        # the shared handler is owned/closed by the bimanual follower AFTER both arms disable torque
