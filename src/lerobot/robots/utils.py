...
def make_robot_from_config(config: RobotConfig) -> Robot:
    ...
    elif config.type == "so101_amazinghand_follower":
        from .so101_amazinghand import SO101AmazingHandFollower

        return SO101AmazingHandFollower(config)
    elif config.type == "so101_amazinghand_right":
        from .so101_amazinghand_right import SO101AmazingHandRight

        return SO101AmazingHandRight(config)
    elif config.type == "so101_amazinghand_left":
        from .so101_amazinghand_left import SO101AmazingHandLeft

        return SO101AmazingHandLeft(config)
    elif config.type == "dual_arm":
        from .dual_arm import DualArm

        return DualArm(config)
