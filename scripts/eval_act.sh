...
if [[ $# -lt 1 ]]; then
    echo "Usage: CAMERAS='<CAMERA_CONFIG_OR_none>' $0 <pretrained-policy-path> [SO-DexARM eval options]" >&2
    exit 2
fi
if [[ -z ${CAMERAS:-} ]]; then
...
POLICY_PATH=$1
shift
EVAL_REPO_ID=${EVAL_REPO_ID:-local/eval_so-dexarm-act}
TASK=${TASK:-SO-DexARM evaluation}
